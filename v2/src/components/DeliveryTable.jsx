...
export default function DeliveryTable({ deliveries, onEdit, onDelete, onStatusChange }) {
  if (deliveries.length === 0) {
    return (
      <div className="text-center py-12 text-slate-500">
        ...
        <p className="mt-1">Add a delivery or adjust your filters.</p>
      </div>
    )
  }
          ...
          <tbody className="divide-y divide-slate-100">
            {deliveries.map((d) => (
              <tr key={d.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-4 py-3 font-medium text-slate-900">{d.patientName}</td>
                <td className="px-4 py-3 text-slate-600 max-w-[200px] truncate">{d.address || '—'}</td>
                <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{d.phone || '—'}</td>
                <td className="px-4 py-3 text-slate-600">
                  {Array.isArray(d.rxNumbers) ? d.rxNumbers.join(', ') : '—'}
                </td>
                <td className="px-4 py-3 font-mono text-xs">
                  {d.trackingNumber ? (
                    <a
                      href={`https://www.ups.com/track?tracknum=${d.trackingNumber}`}
                      target="_blank"
        ...
        {deliveries.map((d) => (
          <div key={d.id} className="bg-white border border-slate-200 rounded-lg p-4">
            <div className="flex items-start justify-between mb-2">
              <h3 className="font-medium text-slate-900">{d.patientName}</h3>
              <StatusBadge status={d.status} />
            </div>
            {d.address && <p className="text-sm text-slate-600 mb-1">{d.address}</p>}
            {d.phone && <p className="text-sm text-slate-600 mb-1">{d.phone}</p>}
            {d.rxNumbers?.length > 0 && (
              <p className="text-sm text-slate-500 mb-1">
                <span className="font-medium">RX:</span> {d.rxNumbers.join(', ')}
              </p>
            )}
            {d.trackingNumber && (
